...
import static com.alibaba.datax.core.util.container.CoreConstant.DATAX_JOB_SETTING_STATIC;
...
public class JobContainer extends AbstractContainer {
    ...
    private void logStatistics() {
        ...
        // 解析出 mongodb| mysql| odps 的 库 , 表
        String sourceDbName = "" ;
        String sourceTableName = "" ;
        String destDbName = "" ;
        String destTableName = "" ;

        Boolean shouldReaderRecord = true ;
        Boolean shouldWriterRecord = true ;

        try{
            if(this.readerPluginName.trim().equals("mongodbreader")){
                sourceDbName = this.configuration.getString(DATAX_JOB_CONTENT_READER_PARAMETER + ".dbName");
                sourceTableName = this.configuration.getString(DATAX_JOB_CONTENT_READER_PARAMETER + ".collectionName");
            }else if(this.readerPluginName.trim().equals("mysqlreader")){
                sourceDbName = this.configuration.getString(DATAX_JOB_CONTENT_READER_PARAMETER +
                        ".mysql_database") ;
                sourceTableName = this.configuration.getString(DATAX_JOB_CONTENT_READER_PARAMETER +
                        ".mysql_table") ;
            }else if(this.readerPluginName.trim().equals("odpsreader")){
                sourceDbName = this.configuration.getString(DATAX_JOB_CONTENT_READER_PARAMETER +
                        ".project") ;
                sourceTableName = this.configuration.getString(DATAX_JOB_CONTENT_READER_PARAMETER +
                        ".table") ;
            }else{
                LOG.info("当前reader:" + this.readerPluginName + " 没有记录读取信息");
                shouldReaderRecord = false ;
            }

            // 解析出 hive db
            if(this.writerPluginName.trim().equals("hdfswriter")){
                destTableName = this.configuration.getString(DATAX_JOB_CONTENT_WRITER_PARAMETER +
                        ".fileName") ;
                String path = this.configuration.getString(DATAX_JOB_CONTENT_WRITER_PARAMETER +
                        ".path") ;
                String [] pathArray = path.split("\\/") ;
                for(String dbName : pathArray){
                    if(dbName.endsWith(".db")){
                        destDbName = dbName.split("\\.")[0] ;
                        break ;
                    }
                }

            }else{
                LOG.info("当前writer:" + this.writerPluginName + " 没有记录写入信息");
                shouldWriterRecord = false ;
            }

        }catch(Exception e){
            LOG.error("解析mongodb| mysql|odps 库,表 异常",e);
        }

        LOG.info("sourceDbName:" + sourceDbName + " sourceTableName:" +
                sourceTableName + " destDbName:" + destDbName + " destTableName:" + destTableName);

       // LOG.info("config:" + this.configuration);
        ...
        String pday = "" ;
        try {
            pday = sdf.format(dateFormat.parse(startTime));
        } catch (ParseException e) {
            LOG.error("pday 日期转换异常",e);
        }
        ...
        long transformSucceedRecords = communication.getLongCounter(CommunicationTool.TRANSFORMER_SUCCEED_RECORDS) ;
        long transformFailedRecords = communication.getLongCounter(CommunicationTool.TRANSFORMER_FAILED_RECORDS) ;
        long transformFilterRecords = communication.getLongCounter(CommunicationTool.TRANSFORMER_FILTER_RECORDS) ;


        // save to mysql
        try{
            if(shouldReaderRecord && shouldWriterRecord){
                Map<String,String> staticMap  = this.configuration.getMap(DATAX_JOB_SETTING_STATIC, String.class) ;
                String table = staticMap.get("table") ;
                Connection connection = connect(staticMap.get("url"),staticMap.get("username"),staticMap.get("password")) ;
                String sql = "insert into "+ table +"(p_day,database_name,table_name,start_time,end_time,execute_seconds," +
                        "total_rows,error_rows,avg_network_flow,write_speed,transform_input_total,transform_output_total) " +
                        "values(?,?,?,?,?,?,?,?,?,?,?,?)" ;
                LOG.info(sql);
                try {
                    PreparedStatement stmt = connection.prepareStatement(sql);
                    stmt.setString(1,pday);
                    stmt.setString(2,sourceDbName);
                    stmt.setString(3,sourceTableName);
                    stmt.setString(4,startTime);
                    stmt.setString(5,endTime);
                    stmt.setLong(6,totalCosts) ;
                    stmt.setLong(7,totalReadRecords);
                    stmt.setLong(8,totalErrorRecords);
                    stmt.setLong(9,byteSpeedPerSecond);
                    stmt.setLong(10,recordSpeedPerSecond);
                    stmt.setLong(11, transformSucceedRecords + transformFailedRecords);
                    stmt.setLong(12,transformSucceedRecords);
                    stmt.execute() ;
                    stmt.close();
                    connection.close();
                } catch (SQLException e) {
                    LOG.error("记录信息保存到 MySQL 异常",e);
                }finally {
                    try {
                        connection.close() ;
                    } catch (SQLException e) {
                        LOG.error("关闭 connection 异常",e);
                    }
                }
            }
        }catch(Exception e){
            LOG.error("记录信息保存异常",e);
        }
        ...
        if (transformSucceedRecords > 0  || transformFailedRecords > 0 || transformFilterRecords > 0) {
            LOG.info(String.format(
                    "\n" + "%-26s: %19s\n" + "%-26s: %19s\n" + "%-26s: %19s\n",
                    "Transformer成功记录总数",
                    transformSucceedRecords,

                    "Transformer失败记录总数",
                    transformFailedRecords,

                    "Transformer过滤记录总数",
                    transformFilterRecords
            ));
        }
